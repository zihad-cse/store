<div class="sidenav" id="sidenav">
    <button class="dropdown-btn">Advanced Kits
        <i class="fa fa-caret-down"></i>
    </button>
    <div class="dropdown-container">
        <a class="sub-dropdown-btn" href="#"><i class="fa fa-caret-down"></i>Link 1</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 2</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 3</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
    </div>
    <div class="border-bottom"></div>
    <button class="dropdown-btn">Antennas
        <i class="fa fa-caret-down"></i>
    </button>
    <div class="dropdown-container">
        <a class="sub-dropdown-btn" href="#"><i class="fa fa-caret-down"></i>Link 1</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 2</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 3</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
    </div>
    <button class="dropdown-btn">Appliance
        <i class="fa fa-caret-down"></i>
    </button>
    <div class="dropdown-container">
        <a class="sub-dropdown-btn" href="#"><i class="fa fa-caret-down"></i>Link 1</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 2</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 3</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
    </div>
    <button class="dropdown-btn">Audio
        <i class="fa fa-caret-down"></i>
    </button>
    <div class="dropdown-container">
        <a class="sub-dropdown-btn" href="#"><i class="fa fa-caret-down"></i>Link 1</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 2</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 3</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
    </div>
    <button class="dropdown-btn">Automatic Light
        <i class="fa fa-caret-down"></i>
    </button>
    <div class="dropdown-container">
        <a class="sub-dropdown-btn" href="#"><i class="fa fa-caret-down"></i>Link 1</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 2</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 3</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
    </div>
    <button class="dropdown-btn">Automatic Switch
        <i class="fa fa-caret-down"></i>
    </button>
    <div class="dropdown-container">
        <a class="sub-dropdown-btn" href="#"><i class="fa fa-caret-down"></i>Link 1</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 2</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 3</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
    </div>
    <button class="dropdown-btn">AVR Microcontroller
        <i class="fa fa-caret-down"></i>
    </button>
    <div class="dropdown-container">
        <a class="sub-dropdown-btn" href="#"><i class="fa fa-caret-down"></i>Link 1</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 2</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 3</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
    </div>
    <button class="dropdown-btn">Battery
        <i class="fa fa-caret-down"></i>
    </button>
    <div class="dropdown-container">
        <a class="sub-dropdown-btn" href="#"><i class="fa fa-caret-down"></i>Link 1</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 2</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 3</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
    </div>
    <button class="dropdown-btn">Biometrics
        <i class="fa fa-caret-down"></i>
    </button>
    <div class="dropdown-container">
        <a class="sub-dropdown-btn" href="#"><i class="fa fa-caret-down"></i>Link 1</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 2</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 3</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
    </div>
    <button class="dropdown-btn">Bluetooth
        <i class="fa fa-caret-down"></i>
    </button>
    <div class="dropdown-container">
        <a class="sub-dropdown-btn" href="#"><i class="fa fa-caret-down"></i>Link 1</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 2</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 3</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
    </div>
    <button class="dropdown-btn">Breadboard
        <i class="fa fa-caret-down"></i>
    </button>
    <div class="dropdown-container">
        <a class="sub-dropdown-btn" href="#"><i class="fa fa-caret-down"></i>Link 1</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 2</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 3</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
    </div>
    <button class="dropdown-btn">Buzzer
        <i class="fa fa-caret-down"></i>
    </button>
    <div class="dropdown-container">
        <a class="sub-dropdown-btn" href="#"><i class="fa fa-caret-down"></i>Link 1</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 2</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 3</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
    </div>
    <button class="dropdown-btn">Cable
        <i class="fa fa-caret-down"></i>
    </button>
    <div class="dropdown-container">
        <a class="sub-dropdown-btn" href="#"><i class="fa fa-caret-down"></i>Link 1</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 2</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 3</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
    </div>
    <button class="dropdown-btn">Capacitor
        <i class="fa fa-caret-down"></i>
    </button>
    <div class="dropdown-container">
        <a class="sub-dropdown-btn" href="#"><i class="fa fa-caret-down"></i>Link 1</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 2</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 3</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
    </div>
    <button class="dropdown-btn">Charger
        <i class="fa fa-caret-down"></i>
    </button>
    <div class="dropdown-container">
        <a class="sub-dropdown-btn" href="#"><i class="fa fa-caret-down"></i>Link 1</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 2</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 3</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
    </div>
    <button class="dropdown-btn">Computer Peripherals
        <i class="fa fa-caret-down"></i>
    </button>
    <div class="dropdown-container">
        <a class="sub-dropdown-btn" href="#"><i class="fa fa-caret-down"></i>Link 1</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 2</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 3</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
    </div>
    <button class="dropdown-btn">Connector
        <i class="fa fa-caret-down"></i>
    </button>
    <div class="dropdown-container">
        <a class="sub-dropdown-btn" href="#"><i class="fa fa-caret-down"></i>Link 1</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 2</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 3</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
    </div>
    <button class="dropdown-btn">Controller
        <i class="fa fa-caret-down"></i>
    </button>
    <div class="dropdown-container">
        <a class="sub-dropdown-btn" href="#"><i class="fa fa-caret-down"></i>Link 1</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 2</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 3</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
    </div>
    <button class="dropdown-btn">Cooler
        <i class="fa fa-caret-down"></i>
    </button>
    <div class="dropdown-container">
        <a class="sub-dropdown-btn" href="#"><i class="fa fa-caret-down"></i>Link 1</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 2</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 3</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
    </div>
    <button class="dropdown-btn">Copper Board
        <i class="fa fa-caret-down"></i>
    </button>
    <div class="dropdown-container">
        <a class="sub-dropdown-btn" href="#"><i class="fa fa-caret-down"></i>Link 1</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 2</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 3</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
    </div>
    <button class="dropdown-btn">Current
        <i class="fa fa-caret-down"></i>
    </button>
    <div class="dropdown-container">
        <a class="sub-dropdown-btn" href="#"><i class="fa fa-caret-down"></i>Link 1</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 2</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 3</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
    </div>
    <button class="dropdown-btn">Crystal Oscillator
        <i class="fa fa-caret-down"></i>
    </button>
    <div class="dropdown-container">
        <a class="sub-dropdown-btn" href="#"><i class="fa fa-caret-down"></i>Link 1</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 2</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 3</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
    </div>
    <button class="dropdown-btn">Data Converter
        <i class="fa fa-caret-down"></i>
    </button>
    <div class="dropdown-container">
        <a class="sub-dropdown-btn" href="#"><i class="fa fa-caret-down"></i>Link 1</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 2</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 3</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
    </div>
    <button class="dropdown-btn">DC Fan
        <i class="fa fa-caret-down"></i>
    </button>
    <div class="dropdown-container">
        <a class="sub-dropdown-btn" href="#"><i class="fa fa-caret-down"></i>Link 1</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 2</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
        <a class="sub-dropdown-btn" href="#">Link 3</a>
        <div class="dropdown-container">
            <a href="">Sub Link 1</a>
            <a href="">Sub Link 2</a>
            <a href="">Sub Link 3</a>
        </div>
    </div>
</div>